...
- **True roll-up recompression** - Roll-ups are declared with an explicit per-range `supersedes` field listing the contained `bN` blocks (e.g. `supersedes: ["b1", "b2"]`). The `compress` tool validates set-equality between `supersedes` and the active blocks fully contained in the range, rejecting missing, duplicated, or unexpected entries. The authored `summary` text is stored verbatim, and accepted roll-ups mark the listed children inactive so only the new parent summary is injected.
...
- **Compressed-block boundary metadata** - synthetic compressed messages are surfaced as `[Compressed section: <topic>]` and include a `<dcp-block-id>bN</dcp-block-id>` tag so later `compress` calls can reference block boundaries by `bN` id.
- **Hierarchical roll-up compression** - compression ranges may now fully contain active compressed blocks, producing a parent block that supersedes those children. Roll-up calls must declare the contained blocks in the per-range `supersedes` array, and partial overlap with active blocks is still rejected.
