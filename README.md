...
When the LLM calls the `compress` tool it provides a `topic` string and one or more `{startId, endId, summary, supersedes?}` ranges. DCP:
...
When compressing a range that fully contains active compressed blocks, the call is a roll-up. The model declares which child blocks are being superseded with the per-range `supersedes` field, which must list each contained active block exactly once using canonical `bN` ids (e.g. `["b1", "b2"]`). DCP validates set-equality between `supersedes` and the blocks fully contained in the requested range; missing, duplicated, or unexpected entries cause the call to be rejected atomically. After acceptance, the listed child blocks are marked inactive so only the new parent summary is injected. The `summary` text is stored verbatim and does not need to reference child blocks inline; the `supersedes` field carries that information.

Examples:

- **Raw compression:** compress `m010..m040` into a new `b7` summary (omit `supersedes`).
- **Roll-up compression:** compress `b7..b9` with `supersedes: ["b7", "b8", "b9"]` and a newly authored parent summary, creating a new parent block and superseding those child blocks.
- **Mixed roll-up compression:** compress a closed range like `m010..m018` that fully contains `b7` with `supersedes: ["b7"]`, so the resulting block covers both the surrounding raw messages and the superseded child block.
